...
def run_ablation(
    ...
    for config_name, ckpt_path in checkpoint_paths.items():
        ...
        model = build_specialist(
            specialist_encoder, pretrained=True, scrwkv_root=scrwkv_root,
            scrwkv_risk_scale=scrwkv_risk_scale,
        ).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--specialist_encoder", type=str, default="resnet34")
    parser.add_argument(
        "--scrwkv_root", type=str,
        default="/home/guest/dinu/SCRWKV/SCRWKV",
    )
    parser.add_argument("--scrwkv_risk_scale", type=float, default=0.25)
    ...
    if args.ablate:
        ...
        model = build_specialist(
            args.specialist_encoder,
            pretrained=True,
            scrwkv_root=args.scrwkv_root,
            scrwkv_risk_scale=args.scrwkv_risk_scale,
        ).to(device)
